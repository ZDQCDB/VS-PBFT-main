...
namespace ns3 {
    ...
    { 
        ...
        int leader_id = (rand() % m_nodeNo);        //主节点随机数选取
        ...
        for (NodeContainer::Iterator i = c.Begin (); i != c.End (); i++)
        {
            ...
            if (j == leader_id) {                   //选中标记为主节点
                ...
                app->is_leader = 0;                 //否则就是普通节点
            }

            app->leader_id= leader_id;              //设置主节点ID
            app->view_number = 1;                   //初始化试图编号
            app->client_id = -1;                    //客户端ID默认为-1
            app->sec_num = 0;                       //安全编号初始化为0
