...
class NodeApp : public Application
{
  public:
    ...
    std::map<Ipv4Address, uint32_t> messageCount;
    ...
    uint32_t m_successfulTxCount;
    uint32_t m_totalTxCount;
    std::map<uint32_t, Time> m_txStartTimes;
    std::map<uint32_t, Time> m_txEndTimes;
    double m_avgLatency;
    // 添加密钥相关成员变量
